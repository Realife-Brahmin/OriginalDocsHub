...
\begin{gather}
	...
	\alpha \sum_{j \in \mathcal{B}} \left((1- \eta_c)P_{c_j}^t + (\frac{1}{\eta_d}-1)P_{d_j}^t )\right) \\
	\begin{align}
		\text{s.t.} & {}\nonumber \\
...
\begin{gather}
    \min_{P_{ij}^t, Q_{ij}^t, v_{j}^t,  q_{D_j}^t, B_{j}^{t+1},
	P_{c_j}^t, P_d{_j}^t, q_{B_j}^t} \quad
	\sum_{\fromto{i, j} \in \mathcal{L}} \left(r_{ij}^t\frac{(P_{ij}^{t})^2 + (Q_{ij}^{t})^2}{v_{0j}^{t}}\right) + 
	\alpha \sum_{j \in \mathcal{B}} \left((1- \eta_c)P_{c_j}^t + (\frac{1}{\eta_d}-1)P_{d_j}^t )\right) \\
	\begin{align}
		\text{s.t.} & {}\nonumber \\
		{p_j^t} & = {\sum P_{jk}^t + \sum \left(P_{ij}^t\right) + P_{d_j}^t - P_{c_j}^t} && \\
		{q_j^t} & = {\sum Q_{jk}^t + \sum \left(Q_{ij}^t\right) + q_{D_j}^t + q_{B_j}^t} && \\
...
\begin{gather}
	...
	\begin{align}
		\text{s.t.} & {}\nonumber \\
		{p_j^t} & = {\sum P_{jk}^t + \sum \left(P_{ij}^t\right)} && \\
		{q_j^t} & = {\sum Q_{jk}^t + \sum \left(Q_{ij}^t\right)} && \\
		{v_j^t} & = {v_{i}^t - 2(r_{ij}^tP_{ij}^t + x_{ij}^tQ_{ij}^t)} &&
	\end{align}
\end{gather}
...
\begin{align}
	{P_{Save}} &= {100\% * \left(1 - \frac{P_{12}}{P_{12} + 
