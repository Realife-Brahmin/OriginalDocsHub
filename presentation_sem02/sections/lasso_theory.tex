...
\label{sec:lasso_theory}
	...
	\begin{frame}[fragile]{Discrete State Space Representation}
		...
		Let $k = 0, 1, \ldots$ and define $x[k]  \triangleq x(kT)$, $u[k] \triangleq \textbf{u}(kT)$ and $y[k] \triangleq y(kT)$. Suppose that $\textbf{u}(t)$ is piece-wise constant during $kT \leq t \leq (k+1)T$. Then
		\begin{alignat}{2}
			content...
		\end{alignat}
	\end{frame}