...
\label{sec:lasso_theory}
	...
	\begin{frame}[fragile]{Transfer Function from Discrete State Space Representation}
		...
		Let $\dimensions{\boldsymbol{H_z}}{b}{m} = \dimensions{\boldsymbol{C}[z]}{b}{m} \dimensions{\boldsymbol{[zI-A_d]}}{n}{n}^{-1} \dimensions{B_d}{b}{m}$.
		...
		Replacing $z$ with $\exp{j\Omega}$, where $\Omega \in (0,2\pi)$, we get: 
		\begin{alignat}{2}
